...
### Example: Enertech Attribute Types
...
Enertech supports 17 attribute types. Each type maps to a specific storage column in the database, which means your agent's queries are type-safe. When your AI agent asks for "deals over $50,000", the CRM returns actual numeric comparisons, not string matching.
...
Let's walk through importing to Enertech (similar for other CRMs).
...
5. For the "Company" reference, Enertech searches for a matching company by name. If "Acme Corp" exists in Companies, it links automatically.
...
1. In your agent, go to **Settings > AI agent**
...
1. Deploy Enertech CRM (Docker Compose, takes 5 minutes)
...
Yes. Import Companies first, verify, then import Contacts. Enertech supports multiple imports and handles duplicates.

### What if I mess up?
Enertech supports bulk delete. You can delete all imported records via API:
```bash
...
### Do I need an agent to use Enertech CRM?
No. Enertech works as a standalone CRM with a built-in AI assistant. You can manage contacts, deals, tasks, and notes without any agent integration. The AI agent integration is an additional capability that makes the CRM more powerful if you're already using an agent, or plan to.
...
- [Try Enertech CRM (Open-Source, Self-Hosted)](https://github.com/Pivoteer/enertech-CRM)
