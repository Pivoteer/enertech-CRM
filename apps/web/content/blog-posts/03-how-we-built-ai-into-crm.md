---
title: "Two Ways AI Works in Enertech CRM: Built-in Assistant and Agent Integration"
slug: "how-we-built-ai-into-crm"
description: "A technical deep-dive into Enertech CRM's two AI systems: a built-in tool-calling assistant with dynamic schema awareness, and external agent integration through the AI agent skill file system."
date: "2026-02-17"
...
# Two Ways AI Works in Enertech CRM: Built-in Assistant and Agent Integration

**Last updated:** February 2026

There are two distinct AI stories in Enertech CRM, and they solve different problems.
...
> add the people from yesterday's meeting to the CRM
  ...
  Creating records in Enertech CRM...
...
export async function buildSystemPrompt(workspaceId: string): Promise<string> {
  ...
  return `You are an AI assistant for Enertech CRM...
...
---
name: enertech
description: Interact with Enertech CRM, manage workspaces,
...
- `ENERTECH_API_URL`: the base URL of your CRM instance (e.g., `https://crm.yourcompany.com`)
- `ENERTECH_API_KEY`: a workspace-scoped API key (prefixed with `oc_sk_`)

API keys are generated in Settings > API Keys inside the CRM. Each key is scoped to a single workspace. All requests include `Authorization: Bearer $ENERTECH_API_KEY`.
...
When you ask your AI agent to do something CRM-related, it recognizes the request matches the `openclaw` skill, reads the skill file, plans a sequence of API calls, executes them over HTTP, and formats the results.
...
```json
{
  "skills": {
    "openclaw": {
      "enabled": true,
      "env": {
        "ENERTECH_API_URL": "https://your-instance.com",
        "ENERTECH_API_KEY": "oc_sk_your_key_here"
      }
    }
  }
}
...
**Agent integration**: go to [Settings > Agent API](/settings/openclaw), generate your skill file, and connect your AI agent. Two minutes. For the full walkthrough, see [How to Connect Your AI agent to Enertech CRM in 2 Minutes](/blog/connect-ai-agent-to-crm).

Both systems work independently. Use one or both. The CRM is the same underneath.

---

[Enertech on GitHub](https://github.com/Pivoteer/enertech-CRM) | [Live Demo](https://enertech-crm.vercel.app/chat) | [Full AI Service Code](https://github.com/Pivoteer/enertech-CRM/blob/main/apps/web/src/services/ai-chat.ts)
